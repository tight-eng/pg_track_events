...
Processed events are stored in JSONL format with each line containing a single event. Files are organized by event name, with events for each type stored in separate files.

The files are named using the pattern: `{timestamp}-{agentID}.jsonl`, where:
...
Raw DB events are stored in JSONL format with each line containing a single event. Files are organized by table name, with events for each table stored in separate files.
